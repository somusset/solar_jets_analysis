...
def csv_export_list(clusters, output):
    '''
        export the list of JetCluster parameters to the output.csv file.
        Inputs
            ------
            clusters : ascii table
                table with JetCluster parameters to be exported
            output : str
                name of the exported csv file
    '''
    ascii.write(clusters, output+'.csv',delimiter=',') 
    
    print(f'The {len(clusters)} JetCluster objects are exported to {output}.csv.')
    return

def csv_import_list(csv_file):
    '''
        import a list of JetCluster parameters from the input_file csv file.
        Inputs
            ------
            csv_file : string
                path or filename to the csv file with JetCluster parameters
        Outputs
            ------
            clusters : ascii table
                table of JetCluster parameters
    '''
    converters = {'date': [ascii.convert_numpy(np.datetime64)], 'flag': [ascii.convert_numpy(str)]}
    clusters = ascii.read(csv_file, converters=converters) 
    
    print(f'The {len(clusters)} JetCluster objects are imported from {csv_file}.')
